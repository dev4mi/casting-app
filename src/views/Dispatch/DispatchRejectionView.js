...
const DispatchRejectionView = () => {
  ...
  const [selectedDispatchNumber, setSelectedDispatchNumber] = useState(null); 
  const [selectedDispatch, setSelectedDispatch] = useState(null); 
  const [records, setRecords] = useState([]);
  const { dispatchDetails, setDispatchDetails, getAllDispatchData, dispatchWithProductsDetails, getSingleDispatchData, getAllDispatchWithProductsData, addDispatchData, updateDispatchData, deleteDispatchMappingRecord, deleteDispatchData } = useContext(DispatchContext);

  useEffect(() => {
    getAllCompanies();
    getAllProducts();
    getAllDispatchWithProductsData();
    getAllDispatchData();
  ...
  const handleEdit = (dispatch) => {
    ...
    setSelectedDispatchNumber(dispatch.dispatch_id); 

    // Populate the form with the selected dispatch record
  ...
  const handleDispatchChange = (event) => {
    setSelectedDispatchNumber(event.target.value);
    console.log('selected:'+event.target.value);
    // let molding = getAllMoldingData(event.target.value);
    const dispatch = dispatchDetails.filter(
      (pp) => pp.dispatch_id === parseInt(event.target.value, 10)
    );
    
    console.log("------"+dispatch[0].dispatch_unique_number);
    handleEdit(dispatch[0]);  
  };
  ...
  const resetForm = async () => {
    ...
    setSelectedDispatchNumber(null)
    await setRecords([]); 
    ...
    try {
      if (validateForm()) {
        ...
        if (editMode && selectedDispatch) {
          ...
          console.log("hi"+ selectedDispatch.companies[0].products[0].rejection_reason)
          await updateDispatchData(selectedDispatch.dispatch_id, 0, 0, records, 'rejection');
            ...
            <Box padding="15px 30px" display="flex" alignItems="center">
              <Box flexGrow={3}>
                <Typography fontSize="18px" fontWeight="500">
                  {editMode ? `Edit Dispatch Data - ${selectedDispatch.heat_number}` : 'Dispatch Form'}
                </Typography>
              </Box>
            </Box>
            <Divider />
              ...
              <Box m={2}>
                <Grid2 size={{ xs: 4, md: 4 }}>
                    <TextField
                      ...
                      name="dispatch_id"
                      variant="outlined"                      
                      label="Select Dispatch Transaction ID"
...
export default DispatchRejectionView;
