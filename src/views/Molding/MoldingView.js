...
const renderParts = (parts) => {
    ...
    if (editMode) {
      ...
      try {
        if(record.company_id != "" && record.product_id != ""){
          ...
          setRecords(records.filter((crecord) => crecord.id !== record.id));
          console.log(records+"-----updated1");
  ...
  const resetForm = () => {
    ...
    const newRecord = {
      ...
      company_id: '',
      product_id: '',
      parts: [], // Empty parts for a fresh form
    };
  
    setRecords(() => []);  // Set new record immediately, no need to set null
    setRecords([newRecord]);
    setSelectedMolding(null); // Clear the selected molding if needed
                            ...
                            <Box mt={2}>
                              <Grid container spacing={2} alignItems="center">
                                {record.parts.map((part, partIndex) => (
                                  <Grid item xs={4} key={part.part_id}>
